...
def top_in_source_fragments(instrument, mode, N=20):
    
    assert instrument in ['orbitrap', 'tof'], "instrument should be either 'orbitrap' or 'tof'"
    assert mode in ['pos', 'neg'], "mode should be either 'pos' or 'neg'"

    this_dir = os.path.dirname(os.path.abspath(__file__))
    frag_lists_by_instrument_by_mode = {
        "orbitrap": {
            "pos": os.path.join(this_dir, 'source_data', 'S2_top_in_source_fragments_orbitrap_pos.tsv'),
            "neg": os.path.join(this_dir, 'source_data', 'S3_top_in_source_fragments_orbitrap_neg.tsv')
        },
        "tof": {
            "pos": os.path.join(this_dir, 'source_data', 'S2_top_in_source_fragments_orbitrap_pos.tsv'),
            "neg": os.path.join(this_dir, 'source_data', 'S3_top_in_source_fragments_orbitrap_neg.tsv')
    ...
    return pd.read_csv(frag_lists_by_instrument_by_mode[instrument][mode], sep = '\t', index_col = 0).head(N)