...
def __harmonize_instrument_mode(instrument, mode):
    """
...
def __retrieve_frequent_deltas(instrument, mode):
    """
    ...
    """
    instrument, mode = __harmonize_instrument_mode(instrument, mode)
    this_dir = os.path.dirname(os.path.abspath(__file__))
    frag_lists_by_instrument_by_mode = {
        "orbitrap": {
            "pos": os.path.join(this_dir, 'source_data', 'S2_top_frequent_delta_mz_orbi_pos.tsv'),
            "neg": os.path.join(this_dir, 'source_data', 'S3_top_frequent_delta_mz_orbi_neg.tsv')
        },
        "tof": {
            "pos": os.path.join(this_dir, 'source_data', 'S4_top_frequent_delta_mz_tof_pos.tsv'),
            "neg": os.path.join(this_dir, 'source_data', 'S5_top_frequent_delta_mz_tof_neg.tsv')
        }
    }
...
def top_N_mz_deltas_for_instrument_and_mode(instrument, ionization_mode, N=20, filter_type=None, sort_by='count_estimate'):
    ...
    """
    mz_deltas = __retrieve_frequent_deltas(instrument, ionization_mode)
    if sort_by:
